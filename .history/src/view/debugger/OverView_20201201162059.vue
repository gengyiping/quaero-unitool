  ...
  methods: {
     handleSelect(key, keyPath) {
      ...
      readloacl(){
        debugger
        var _this=this
        _this.stompClient.send('/app/readloacl')
      ...
      writeloacl(){
         ...
         _this.stompClient.send('/app/writeloacl/'+ _this.ip)
          Bus.$on('writeloacl',function(val){//监听fi
          debugger
           var ips=val.split(':')
            _this.ip=ips[0]
            _this.port=ips[1]
      ...
      readEquipment(){
         var _this=this
        _this.stompClient.send('/app/readEquipment')
      ...
      writeEquipment(){
         var _this=this
        _this.stompClient.send('/app/writeEquipment/'+ _this.ip)
        Bus.$on('writeEquipment',function(val){//监听fi
           var ips=val.split(':')
            _this.ip=ips[0]
            _this.port=ips[1]
         })
