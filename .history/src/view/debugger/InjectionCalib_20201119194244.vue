       ...
       for(var i=0;i<kk.length;i++){
        ...
        Bus.$on('searchMotor',(response) => {
          debugger
            ...
            for(let key in obj) {
                              if(key=='forward')_this.kk[i].forward =obj[key]
                              if(key=='reverse')_this.kk[i].reverse=ob1[key]
                              if(key=='zeroFlag')_thiskk[i].zero=obj[key]
                              if(key=='zero')_this.kk[i].zeroCoord=obj[key]
                              if(key=='coord')_this.kk[i].coord=obj[key]
           }
         })
