       ...
       for(var i=0;i<kk.length;i++){
        ...
        Bus.$on('searchMotor',function(val){//监听first组件的txt事件
           ...
           let obj=JSON.parse(val)
            for(let key in obj) {
                              if(key=='forward')kk[i].forward =obj1[key1]
                              if(key=='reverse')kk[i].reverse=obj1[key1]
                              if(key=='zeroFlag')kk[i].zero=obj1[key1]
                              if(key=='zero')kk[i].zeroCoord=obj1[key1]
                              if(key=='coord')kk[i].coord=obj1[key1]
           }
        })
     }
