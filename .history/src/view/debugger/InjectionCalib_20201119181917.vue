           ...
           let obj=JSON.parse(val)
            for(let key in obj) {
              // debugger
                ...
                if(key==forward)motorinfo.forward =obj[key]
                if(key==reverse)motorinfo.reverse=obj[key]
                if(key==zeroFlag)motorinfo.zero=obj[key]
                if(key==zero)motorinfo.zeroCoord=obj[key]
                if(key==coord)motorinfo.coord=obj[key]
           }
        })
