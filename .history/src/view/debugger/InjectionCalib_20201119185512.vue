               ...
               for(var i=0;i<kk.length;i++){
                   if(kk[i].motorId==key){
                         for(let key1 in obj[key]) {
                              if(key1==forward)kk[i].forward =obj[key][key1]
                              if(key1==reverse)kk[i].reverse=obj[key][key1]
                              if(key1==zeroFlag)kk[i].zero=obj[key][key1]
                              if(key1==zero)kk[i].zeroCoord=obj[key][key1]
                              if(key1==coord)kk[i].coord=obj[key][key1]
                           }
                         }
               }  
                   }
               }
                // if(key==forward)_this.motorinfo[i].forward =obj[key]
                // if(key==reverse)_this.motorinfo[i].reverse=obj[key]
                // if(key==zeroFlag)_this.motorinfo[i].zero=obj[key]
                // if(key==zero)_this.motorinfo[i].zeroCoord=obj[key]
                // if(key==coord)_this.motorinfo[i].coord=obj[key]
           }
        })
    }
  }
