...
export default {
  ...
  methods: {
     handleSelect(key, keyPath) {
      ...
      readCoord(opt){//0 读取坐标参数 ;1读取设备坐标参数 2读取坐标 ;3读取设备坐参数
       var _this = this
       _this.coord=''
         this.getCoordPoint()
       ...
       writeCoord(opt){//0 读取坐标 ;1读取设备坐标
       var _this = this;
         this.getCoordPoint()
          this.stompClient.send('/app/writeCoord/'+opt+'/'+this.motorId+'/'+this.coodPoint)
           Bus.$on('writeCoord',function(val){//监听fi
           debugger
              _this.coord=val
          })
      },
