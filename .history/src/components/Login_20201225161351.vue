       ...
       var _this=this
        ...
        this.stompClient.connect(userinfo, function (frame) {
          ...
          _this.stompClient.subscribe('/user/initInfo/alone/getResponse', (val) => {//连接信息
           ...
           _this.stompClient.subscribe('/user/initstate/alone/getResponse', (val) => { //获取上一次页面状态
          console.log(val.body)
               _this.getData(val.body)
              var pagesindexs=_this.returnobj.message.split("-")
              _this.$store.state.rootPage=_this.returnobj.message
              _this.$store.state.firstPage=_this.returnobj.message
        });
        _this.stompClient.subscribe('/user/ifinit/alone/getResponse', (val) => { //获取是否初始化连接设备
          console.log(val.body)
               var obj = JSON.parse(val.body);
                _this.returnobj=obj.data
                 Bus.$emit('ifinit',_this.returnobj.message)
                _this.$store.state.resinfo=this.returnobj.message
        });
