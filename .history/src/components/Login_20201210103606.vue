...
export default {
  ...
  methods: {
    ...
    loaduser(){
        this.getUser()
          if(this.user.username!=''){//cookie中有user填充输入框
    ...
    config() {
     ...
     sub(){
       var _this=this
        this.createUser();
        this.stompClient.connect(_this.user, function (frame) {
            console.log('-------连接成功------------')
     ...
     createUser(){//创建用户
         ...
         var userinfo = this.getUser()
          if (userinfo !== '') {
            ...
            this.user.projectName= userinfo.projectName
          }else{
            this.user.username =loginform.username
            this.user.password = loginform.password
            this.user.projectName= loginform.projectName
          }
     },
     getUser(){
       debugger
     ...
     cacheUser() {
       this.$cookieStore.setCookie( 'user' , this.user)
      },
