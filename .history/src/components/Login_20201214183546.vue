    ...
    config() {
     ...
     sub(){
       ...
       var _this=this
        ...
        _this.stompClient.subscribe('/user/grooveMove/alone/getResponse', function (res) {//走计数凹槽
             _this.getData(val.body)
               _this.$message('success:' + _this.returnobj.message)
        });
        _this.stompClient.subscribe('/user/searchInlineopt/alone/getResponse', function (res) {//读取电机参数
            console.log("----searchInlineopt----")
                 _this.coord=res.body
                  Bus.$emit('searchInlineopt',_this.coord)
               //  _this.centerDialogVisible =true
        });
         _this.stompClient.subscribe('/user/writeInlineopt/alone/getResponse', function (res) {//写入电机参数
