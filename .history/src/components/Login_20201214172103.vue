    ...
    config() {
     ...
     sub(){
       ...
       var _this=this
        ...
        _this.stompClient.subscribe('/user/transferMove/alone/getResponse', function (res) {//区域转移
             _this.getData(val.body)
            _this.$message('success:' + _this.returnobj.message)
        });
         _this.stompClient.subscribe('/user/openBoot/alone/getResponse', function (res) {//开启固件更新
             _this.getData(val.body)
            _this.$message('success:' + _this.returnobj.message)
        });
