       ...
       var _this=this
        ...
        _this.stompClient.subscribe('/user/deleteCoord/alone/getRespons', function (val) {//删除设备参数
         ...
         _this.stompClient.subscribe('/user/readCoord/alone/getResponse', function (val) {//读取参数
              _this.getData(val.body)
                Bus.$emit('readCoord',__this.returnobj.message)
        });
        ...
        _this.stompClient.subscribe('/user/readCcoordLimit/alone/getResponse', function (val) {//读取限制坐标
            console.log("----coordLimit----")
              _this.getData(val.body)
              Bus.$emit('phymax',_this.returnobj)
        });
         _this.stompClient.subscribe('/user/writeCcoordLimit/alone/getResponse', function (val) {//写入限制坐标
            console.log("----coordLimit----")
              _this.getData(val.body)
              _this.$message('success:' + _this.returnobj.message)
        });
        _this.stompClient.subscribe('/user/readOutStepForm/alone/getResponse', function (res) {//读取失步数
            console.log("----outStepForm----")
               _this.getData(val.body)
                    Bus.$emit('outMove',_this.returnobj.message)
        });
        _this.stompClient.subscribe('/user/writeOutStepForm/alone/getResponse', function (res) {//设置失步数
            console.log("----outStepForm----")
               _this.getData(val.body)
                 _this.$message('success:' + _this.returnobj.message)
        });
        _this.stompClient.subscribe('/user/readZeroOpt/alone/getResponse', function (res) {
            console.log("----zeroOpt----")
             _this.getData(val.body)
                    Bus.$emit('zeroOpt',_this.returnobj.message)
        });
        _this.stompClient.subscribe('/user/writeZeroOpt/alone/getResponse', function (res) {
            console.log("----zeroOpt----")
             _this.getData(val.body)
              _this.$message('success:' + _this.returnobj.message)
        });
         _this.stompClient.subscribe('/user/createMotorFile/alone/getResponse', function (res) {
