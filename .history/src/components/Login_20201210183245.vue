...
debugger
     ...
     sub(){
       ...
       var _this=this
        ...
        this.stompClient.connect(userinfo, function (frame) {
          ...
          _this.stompClient.subscribe('/topic/status', function (data) {
            debugger
            var obj = _thisgetData(data.body)
            handleMessage(obj);
    ...
    switch (data.type) {
        case 'USER':
            showUserMsg(data);//显示用户消息
            break;
        case 'SYSTEM':  
            showSystemMsg(msg);//系统消息
            break;
        // case 'REVOKE':
        //     showRevokeMsg(data);
        //     break;
