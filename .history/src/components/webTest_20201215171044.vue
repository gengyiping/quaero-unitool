<template>
  ...
  :with-header="false">
  <span>信息栏</span>
       <!-- <li  v-for="item in infoOptions"
                          :key="item.label"
                          :label="item.label"
                          >{{item.label}}</li> -->
    <!-- <ul class="infinite-list" v-infinite-scroll="resinfo" style="overflow:auto">
    </ul> -->
    <el-scrollbar
            style="height: 100%"
            wrap-class="scrollbar-wrapper"  id="data-list-content">
            <div
              ...
              <div v-for="i in infolabel" :key="i" class="infinite-list-item">{{ i }}</div>
            </div>
          </el-scrollbar>
</el-drawer>
