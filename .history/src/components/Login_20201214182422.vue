     ...
     sub(){
       ...
       var _this=this
        ...
        _this.stompClient.subscribe('/user/searchMotor/alone/getResponse', function (val) {//查询所有电机信息
         ...
         _this.stompClient.subscribe('/user/steplost/alone/getResponse', function (val) {
             _this.getData(val.body)
               let reg=new RegExp(';','g')//g代表全部
                let newMsg=_this.returnobj.message.replace(reg,'<br/>')
                 _this.$message("当前坐标："+JSON.parse(newMsg))
        });
