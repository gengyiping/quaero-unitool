        ...
        this.stompClient.connect(userinfo, function (frame) {
          debugger
           ...
           _this.stompClient.subscribe('/topic/getResponse', (val) => {
             ...
             _this.stompClient.subscribe('/topic/chatRoom', function (data) {
            handleMessage(getData(data.body));
        });

        // 本地订阅
         _this.stompClient.subscribe('/user/' + uid + '/chat', function (data) {
            handleMessage(getData(data.body));
        });

        // 错误信息订阅
         _this.stompClient.subscribe('/user/' + uid + '/error', function (data) {
            getData(data.body);
        });

          _this.stompClient.subscribe('/topic/status', function (data) {//连接状态
            debugger
