     ...
     sub(){
       ...
       var _this=this
        ...
        _this.stompClient.subscribe('/user/searchMotor/alone/getResponse', function (val) {//查询所有电机信息
         ...
         _this.stompClient.subscribe('/user/getCoord/alone/getResponse', function (res) {//获取当前坐标
           _this.getData(val.body)
             _this.$message("当前坐标："+JSON.parse(_this.returnobj.message))
        });
         _this.stompClient.subscribe('/user/steplost/alone/getResponse', function (res) {
             _this.getData(val.body)
