...
export default {
  ...
  methods: {
    ...
    //  this.connection()
      // 需要有一个失败重连得到问题
      this.socket = new SockJS(this.socketUrl,{},50000)// 连接服务端
...
//下面会报错，应该是json的问题，但是数据可以接收到
        ...
        this.stompClient.subscribe('/user/writeloacl/alone/getResponse', (val) => {//读取本地Ip信息
          // this.list1 = JSON.parse(val.body)
            debugger
             console.log(val.body)
         ...
         this.stompClient.subscribe('/user/readloacl/alone/getResponse', (val) => {//写入本地Ip信息
          // this.list1 = JSON.parse(val.body)
            debugger
             console.log(val.body)
             _this.allmotor = val.body
